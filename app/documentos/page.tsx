...
import { getDocumentos, clearDocumentos, DocResult } from '@/lib/api'
import { CheckCircle, AlertCircle, XCircle, RefreshCw, Clock, Trash2 } from 'lucide-react'
...
function fmtCLP(n: number|null|undefined) {
  return (n!=null && typeof n==='number') ? `$ ${n.toLocaleString('es-CL')}` : '—'
}
...
export default function Documentos() {
  ...
  const cargar = () => setDocs(getDocumentos())
  useEffect(() => { cargar() }, [])

  const borrar = () => { clearDocumentos(); setDocs([]) }
    ...
    <div style={{ padding:'32px 36px', maxWidth:1100 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          {docs.length > 0 && (
            <button onClick={borrar} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', background:'white', border:'1px solid var(--rojo-bd)', borderRadius:8, fontSize:12, cursor:'pointer', fontFamily:'Sora,sans-serif', color:'var(--rojo)' }}>
      ...
      <div style={{ background:'white', borderRadius:16, border:'1px solid var(--gray-200)', overflow:'hidden' }}>
        {filtrados.length===0 ? (
          <div style={{ padding:60, textAlign:'center', color:'var(--gray-400)' }}>
            <p style={{ margin:0, fontSize:13 }}>No hay documentos{filtro!=='todos'?` en zona ${ZONA_LABEL[filtro]}`:''}. Sube facturas desde el Dashboard.</p>
          </div>
        ) : (
            ...
            <tbody>
              {filtrados.map((doc,i)=>(
                <tr key={doc.doc_id||i} style={{ borderBottom:'1px solid var(--gray-100)', background:i%2===0?'white':'var(--gray-50)' }}>
                  ...
                  <td style={{ padding:'12px 16px', fontFamily:'IBM Plex Mono,monospace', fontSize:12, color:'var(--gray-600)' }}>{doc.folio?`N° ${doc.folio}`:'—'}</td>
                  <td style={{ padding:'12px 16px', fontFamily:'IBM Plex Mono,monospace', fontWeight:600, color:ZONA_COLOR[doc.zona] }}>{fmtCLP(doc.total_clp)}</td>
                  <td style={{ padding:'12px 16px', fontSize:12, color:'var(--gray-400)', display:'flex', alignItems:'center', gap:4 }}><Clock size={11} />{fmtDate(doc.timestamp)}</td>
                  ...
                  <td style={{ padding:'12px 16px', fontSize:11, color:'var(--gray-400)', maxWidth:200 }}>
                    <span style={{ display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{doc.accion}</span>
                  </td>
                </tr>
              ))}
            </tbody>
