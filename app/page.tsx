...
function fmtCLP(n: number|null|undefined) {
  return (n != null && typeof n === 'number') ? `$ ${n.toLocaleString('es-CL')} CLP` : '—'
}
...
export default function Dashboard() {
  ...
  useEffect(() => { setDocs(getDocumentos()) }, [])
  ...
  const procesarArchivo = useCallback(async (file: File) => {
    ...
    setError(null); setResultado(null); setIaAnalisis(''); setIaRes('')
    setArchivo(file.name); setProcesando(true)
  ...
  const borrarHistorial = () => { clearDocumentos(); setDocs([]); setResultado(null) }
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28 }}>
        <div>
          ...
          <p style={{ margin:'4px 0 0', fontSize:13, color:'var(--gray-400)' }}>
            Sube una factura y observa el pipeline en tiempo real
          </p>
        </div>
        {docs.length > 0 && (
          <button onClick={borrarHistorial}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', background:'white', border:'1px solid var(--rojo-bd)', borderRadius:8, fontSize:12, cursor:'pointer', color:'var(--rojo)', fontFamily:'Sora,sans-serif' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginTop:24 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={{ background:'white', borderRadius:16, padding:24, border:'1px solid var(--gray-200)' }}>
            ...
            {error && <div style={{ marginTop:12, padding:'10px 14px', background:'var(--rojo-bg)', border:'1px solid var(--rojo-bd)', borderRadius:8, fontSize:12, color:'var(--rojo)' }}>{error}</div>}
          </div>
              ...
              <div className={`result-${resultado.zona} fade-up`} style={{ borderRadius:12, padding:18 }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ fontSize:14, fontWeight:600, color:ZONA_COLOR[resultado.zona] }}>Zona {ZONA_LABEL[resultado.zona]}</span>
                  </div>
                  <ZonaBadge zona={resultado.zona} />
                </div>

                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:14 }}>
                  {([['Proveedor',resultado.proveedor??'—'],['RUT',resultado.rut??'—'],['Folio',resultado.folio?`N° ${resultado.folio}`:'—'],['Total',fmtCLP(resultado.total_clp)],['Emisión',resultado.fecha_emision??'—'],['ID',resultado.doc_id]] as [string,string][]).map(([k,v])=>(
                ...
                {resultado.email_enviado && (
                  <p style={{ margin:0, fontSize:11, color:'var(--gray-400)', display:'flex', alignItems:'center', gap:4 }}>
                    <CheckCircle size={11} /> Notificación por email enviada
                  </p>
                )}
                ...
                {(iaLoading || iaRes || iaAnalisis) && (
                  <div style={{ marginTop:12, borderTop:'1px solid rgba(0,0,0,0.06)', paddingTop:12 }}>
                    <p style={{ margin:'0 0 8px', fontSize:10, fontWeight:600, color:ZONA_COLOR[resultado.zona], textTransform:'uppercase', letterSpacing:'0.06em' }}>✦ Análisis con IA</p>
                    {iaLoading && <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'var(--gray-400)' }}><span className="spinner" style={{ width:12, height:12, flexShrink:0 }} />Analizando con Gemini…</div>}
                    {iaRes && !iaLoading && (
                      <div style={{ background:'rgba(255,255,255,0.55)', borderRadius:8, padding:'8px 10px', marginBottom:8 }}>
                        ...
                        <p style={{ margin:0, fontSize:12, lineHeight:1.6 }}>{iaRes}</p>
                      </div>
                    )}
                    {iaAnalisis && !iaLoading && (
                      <div style={{ background:'rgba(255,255,255,0.55)', borderRadius:8, padding:'8px 10px' }}>
                        ...
                        <p style={{ margin:0, fontSize:12, lineHeight:1.6 }}>{iaAnalisis}</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              {docs.slice(0,5).map((doc,i)=>(
                <div key={doc.doc_id} style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 0', borderBottom:i<Math.min(docs.length,5)-1?'1px solid var(--gray-100)':'none' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <p style={{ margin:0, fontSize:11, color:'var(--gray-400)', fontFamily:'IBM Plex Mono,monospace' }}>{fmtCLP(doc.total_clp)}</p>
                  </div>
...
function KPIBar({ docs }: { docs: DocResult[] }) {
